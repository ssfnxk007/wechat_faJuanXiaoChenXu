    ...
    <div class="login-card">
      <section class="login-visual">
        <div class="login-visual-copy">
          <span class="page-kicker login-kicker">发券系统后台</span>
          <h1>零售发券后台管理系统</h1>
          <p>面向小程序发券、券包售卖、支付回调、用户券管理与 ERP 核销的一体化后台入口。</p>
          <ul>
            <li>支持微信获取 OpenId 后自动生成用户</li>
            <li>支持新人券、无门槛券、指定商品券和满减券</li>
            <li>支持公开领取、指定发放、券包售卖与支付发券</li>
      ...
      <section class="login-panel">
        <div class="login-panel-head">
          <span class="page-kicker">欢迎登录</span>
          <h1>进入后台</h1>
          <p>当前已接通后台 JWT 登录，可先使用开发管理员账号进入系统。</p>
        </div>

        <form class="login-form" @submit.prevent="goDashboard">
          <input v-model="form.username" type="text" placeholder="请输入账号" />
          <input v-model="form.password" type="password" placeholder="请输入密码" />
          <button type="submit" class="primary-button login-submit">进入后台</button>
        </form>

        <div class="login-note">
          默认开发账号：<code>admin</code> / <code>123456</code>。后续可继续切换到数据库权限体系与动态菜单。
        </div>
...
const goDashboard = async () => {
  try {
    ...
    errorMessage.value = getErrorMessage(error, '登录失败，请检查账号密码后重试')
